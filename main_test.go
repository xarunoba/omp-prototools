...
func TestGetCachedData(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() string {
				...
				data := CachedData{
					Entries: map[string]DirectoryCacheData{
						"test-hash": {
							StatusData: map[string]ToolStatus{"go": {ResolvedVersion: "1.26.0", IsInstalled: true}},
							Timestamp:  now,
						},
					},
				}
		...
		{
			name: "valid outdated cache",
			setup: func() string {
				cacheFile := filepath.Join(t.TempDir(), "cache.json")
				data := CachedData{
					Entries: map[string]DirectoryCacheData{
						"test-hash": {
							OutdatedData: map[string]OutdatedStatus{"node": {IsOutdated: true}},
							Timestamp:    now,
						},
					},
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer func() { getDirectoryContext = oldGetDirectoryContext }()

			oldForceRefresh := forceRefresh
			defer func() { forceRefresh = oldForceRefresh }()

			getCacheFile = func() string { return tt.setup() }
			getDirectoryContext = func(configMode string) (string, error) { return "test-hash", nil }
			forceRefresh = tt.forceRefresh

			data, ok := getCachedData(tt.config, tt.config.ConfigMode)
			if ok != tt.wantOk {
...
func TestGetToolStatus(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "cache hit - valid status data",
			setupCache: func() string {
				cacheFile := filepath.Join(t.TempDir(), "cache.json")
				data := CachedData{
					Entries: map[string]DirectoryCacheData{
...
func TestGetOutdatedStatus(t *testing.T) {
	now := time.Now().Unix()
	...
	tests := []struct {
		...
		{
			name: "cache hit - valid outdated data",
			setupCache: func() string {
				cacheFile := filepath.Join(t.TempDir(), "cache.json")
				data := CachedData{
					Entries: map[string]DirectoryCacheData{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantToolCount > 0 {
				mockOutput = `{"node": {"is_outdated": true}, "go": {"is_outdated": false}}`
			}
			runProtoCommand = func(args []string) ([]byte, error) {
				return []byte(mockOutput), nil
			}

			tools := getOutdatedStatus(tt.config)

			if len(tools) != tt.wantToolCount {
				t.Errorf("getOutdatedStatus() got %d tools, want %d", len(tools), tt.wantToolCount)
			}
			...
			if tt.wantOutdated {
				...
				if !hasOutdated {
					t.Error("getOutdatedStatus() expected at least one outdated tool")
				}
			}
		})
	}
